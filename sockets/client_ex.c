...
int main(int argc, char *argv[])
{
  ...
  /* Tell our socket to be oclient_ex.c:73:1: note: each undeclared identifier is reported only once for each function it appears inf the internet family (AF_INET). */
...
/* Open our file for copying to the socket. */
  ...
  while ((num_char = read(s, ch, MAXLINE)) > 0)
    if (write(1, ch, num_char) < num_char)/* Open our file for copying to the socket. */
    bbf = open(BBSD_FILE, O_RDONLY);
    if (bbf == -1)
      write(sfd, "No information, dude.\n", strlen("No information, dude.\n"));
    else
